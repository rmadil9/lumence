...
2. **Next.js front-end plus a Node backend.** One language everywhere and shared types across the
...
3. **Next.js front-end plus a FastAPI (Python) backend — chosen.** Adil already knows Python. It is
...
| | Next.js front-end | FastAPI backend |
|---|---|---|
| Serves | The browser | The front-end |
...
- **Better Auth stays in the front-end.** Sign-in, verification, password reset and Google OAuth are
  browser-facing concerns, which is exactly what a front-end is for. The 15-day stateless JWT
...
- **Postgres is reached only by the backend.** The front-end never opens a database connection.
- **How the front-end authenticates to the backend is decided in
  [ADR 0017](0017-bff-to-backend-authentication.md):** a service credential proves the caller is
  our front-end, and the user's token is forwarded unchanged for the backend to verify itself.
...
- **`04-contracts.md` needs revising and is signed off**, so this ADR is what authorises that.
  It currently describes one boundary, browser to server. There are now two: browser to front-end, and
  front-end to backend. **Adil drives that revision** — he is the backend developer and the first pass
...
- **The front-end will try to grow business logic.** This is the standard failure of the pattern and it
...
- **Spec A7 — a user seeing only their own data — now has to hold across two services.** The
  backend must never trust a user identifier the front-end sends without that being a deliberate,
